...
class AddTestcase(unittest.TestCase, Appium):
    ...
    # 编写测试用例前先简单了解下 picture.py 内用户操作和其他类方法(不需要全部记下),编写用例过程中可随时翻阅
    # 用例流程:被测功能(对象)--->测试步骤(预期结果)--->用户操作(最好加以注释)--->操作后等待--->操作结果断言(output输出结果)
    # 操作后等待的一点说明:现阶段请使用 time.sleep() 方法执行等待,其他等待方法(更科学,高效)需完善测试用例编写方案后启用
    def test_template(self):
        ...
        """
        # 点击立即体验按钮
        self.wait_element('com.excelliance.dualaid:id/bt_explore', 15, 1)
        self.find_element('com.excelliance.dualaid:id/bt_explore').click()
        time.sleep(5)
        try:
            self.assertIn('com.excelliance.dualaid:id/first_start_ok', self.page_source())
