...
class ForwardSetting(object):
    ...
    def install_apk(self):
        ...
        if self.phone == 'oppoR7':
            while True:
                try:
                    if d(resourceId="com.android.packageinstaller:id/apk_info_view").exists(10):
...
class StartTimeTest(object):
            ...
            if self.phone == 'oppoR7':
                try:
                    if kind == 'normal':
                        ...
                        if d(resourceId='com.excelliance.dualaid:id/add_but').exists(15):
                            d.press('back')
                            time.sleep(3)
                            d.app_start(pkg_name)
                        else:
        ...
        def set_step():
            while True:
                if d(text=u'点击加号，添加双开应用').exists(15):
                    ...
                    d.screenshot(os.path.join(os.path.abspath(os.path.dirname(__file__)),
                                              'error\set_app_status_error%s.png' % time.strftime('%Y%m%d_%H%M%S')))
                    d.app_stop(pkg_name)
                    time.sleep(2)
                    d.app_start(pkg_name)
            try:
    ...
    def data_handle(self):
        ...
        # 定义邮件正文内容
        global mail_content, email_content_flag
...
# 执行测试
def run_start_time(state, n=3):
    global thread, t, email_content_flag, thread_flag, d
    d = atx_init()
