...
# 文件路径
prj_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # 项目路径(根目录)
test_case_path = os.path.join(prj_path, 'test_case', 'single_case')  # 用例路径
log_file = os.path.join(prj_path, 'log', 'log_{}.txt'.format(today))  # log存放路径
report_file = os.path.join(prj_path, 'report')  # 测试报告存放路径
data_file = os.path.join(prj_path, 'test_data', 'test_data.xls')  # 测试数据存放路径
